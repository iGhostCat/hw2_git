from src.csv_pd_converter import csv_to_list_of_dicts, excel_to_list_of_dicts
from src.utils import json_to_list
from src.processing import filter_by_state, sort_by_date
from src.generators import filter_by_currency
from src.reg_exp import reg_proc_search
import re


def main (database_path = ' ',
          file_type = 'json',
          trans_status = 'EXECUTED',
          date_filter = 'нет',
          date_ascending = False,
          ruble_filter = 'нет',
          query_search = 'нет',
          query_phrase = 'Перевод'):
    success_proc_choice = False
    transactions_data = []
    while not success_proc_choice:

        file_type = int(input('''Привет! Добро пожаловать в программу работы с банковскими транзакциями.\nВыберите необходимый пункт меню:
                    1. Получить информацию о транзакциях из JSON-файла
                    2. Получить информацию о транзакциях из CSV-файла
                    3. Получить информацию о транзакциях из XLSX-файла'''))
        if file_type == 1:
            print('Для обработки выбран JSON-файл.')
            transactions_data = json_to_list(database_path)
            success_proc_choice = True
        elif file_type == 2:
            print('Для обработки выбран CSV-файл.')
            transactions_data = csv_to_list_of_dicts(database_path)
            success_proc_choice = True
        elif file_type == 3:
            print('Для обработки выбран XLSX-файл.')
            transactions_data = excel_to_list_of_dicts(database_path)
            success_proc_choice = True
        else:
            print('Неверный ввод варианта обработки!')

    success_status_choice = False
    while not success_status_choice:
        print('''Введите статус, по которому необходимо выполнить фильтрацию. 
Доступные для фильтровки статусы: EXECUTED, CANCELED, PENDING
''')
        trans_status = str(input())
        if trans_status.upper() == 'EXECUTED':
            print('Операции отфильтрованы по статусу "EXECUTED"')
            transactions_data = filter_by_state(transactions_data,'EXECUTED')
            success_status_choice = True
        elif trans_status.upper() == 'CANCELED':
            print('Операции отфильтрованы по статусу "CANCELED"')
            transactions_data = filter_by_state(transactions_data,'CANCELED')
            success_status_choice = True
        elif trans_status.upper() == 'PENDING':
            print('Операции отфильтрованы по статусу "PENDING"')
            transactions_data = filter_by_state(transactions_data,'PENDING')
            success_status_choice = True
        else:
            print('Выбранный статус недоступен, выберите другой статус!')
            success_status_choice = False

    success_date_filter_choice = False
    while not success_date_filter_choice:
        print("Отсортировать операции по дате? Да/Нет")
        date_filter = str(input())
        if date_filter.lower() == 'да':
            success_date_filter_choice = True
        elif date_filter.lower() == 'нет':
            success_date_filter_choice = True
        else:
            print('Неверный ввод! Попробуйте ещё раз')
            success_date_filter_choice = False

    ascending_choice = False
    while not ascending_choice:
        print("Отсортировать по возрастанию или по убыванию?")
        asc = str(input())
        if asc.lower() == 'по возрастанию':
            ascending_choice = True
            date_ascending = True
        elif asc.lower() == 'по убыванию':
            ascending_choice = True
            date_ascending = False
        else:
            print('Неверный ввод! Попробуйте ещё раз')
            ascending_choice = False

    transactions_data = sort_by_date(transactions_data, date_ascending)

    ruble_choice = False
    while not ruble_choice:
        print("Выводить только рублёвые транзакции? Да/Нет")
        ruble_or_not = str(input())
        if ruble_or_not.lower() == 'да':
            ruble_choice = True
            transactions_data = list(filter_by_currency(transactions_data,'RUB'))
        elif ruble_or_not.lower() == 'нет':
            ruble_choice = True
        else:
            print('Неверный ввод! Попробуйте ещё раз')
            ruble_choice = False

    query_choice = False
    while not query_choice:
        print("Отфильтровать список транзакций по определенному слову в описании? Да/Нет")
        query_search = str(input())
        if query_search.lower() == 'да':
            query_phrase = str(input("Введите слово или фразу для поиска:"))
            transactions_data = reg_proc_search(transactions_data, query_phrase, True, re.IGNORECASE)
            query_choice = True

        elif query_search.lower() == 'нет':
            query_choice = True
        else:
            print('Неверный ввод! Попробуйте ещё раз')
            query_choice = False
    if not transactions_data:
        print('''Не найдено ни одной транзакции, подходящей под ваши условия фильтрации''')
    return transactions_data



#############################################
print(main('../data/transactions_excel.xlsx'))







###########################################################

#DEEPSEEK VERSION OF MAIN.PY

##########################################################

'''from typing import List, Dict, Optional
import re
from unittest.mock import patch
from src.csv_pd_converter import csv_to_list_of_dicts, excel_to_list_of_dicts
from src.utils import json_to_list
from src.processing import filter_by_state, sort_by_date
from src.generators import filter_by_currency
from src.reg_exp import reg_proc_search
import re

def main(database_path: str = None) -> List[Dict]:
    """
    Главная функция обработки транзакций с интерактивным меню.
    Возвращает отфильтрованный список транзакций.
    """
    print("Привет! Добро пожаловать в программу работы с банковскими транзакциями.")

    # 1. Выбор типа файла
    transactions = select_file_type(database_path)
    if not transactions:
        print("Не удалось загрузить транзакции.")
        return []

    # 2. Фильтрация по статусу
    transactions = filter_by_status_interactive(transactions)

    # 3. Сортировка по дате
    transactions = sort_by_date_interactive(transactions)

    # 4. Фильтрация по рублям
    transactions = filter_by_currency_interactive(transactions)

    # 5. Поиск по описанию
    transactions = search_by_description_interactive(transactions)

    # 6. Вывод результатов
    print_transactions(transactions)

    return transactions


def select_file_type(database_path: str = None) -> List[Dict]:
    """Интерактивный выбор типа файла"""
    while True:
        print("Выберите необходимый пункт меню:")
        print("1. Получить информацию о транзакциях из JSON-файла")
        print("2. Получить информацию о транзакциях из CSV-файла")
        print("3. Получить информацию о транзакциях из XLSX-файла")

        choice = input().strip()

        if not database_path:
            database_path = input("Введите путь к файлу: ").strip()

        try:
            if choice == "1":
                print("Для обработки выбран JSON-файл.")
                return json_to_list(database_path)
            elif choice == "2":
                print("Для обработки выбран CSV-файл.")
                return csv_to_list_of_dicts(database_path)
            elif choice == "3":
                print("Для обработки выбран XLSX-файл.")
                return excel_to_list_of_dicts(database_path)
            else:
                print("Неверный ввод! Пожалуйста, выберите 1, 2 или 3.")
        except Exception as e:
            print(f"Ошибка при загрузке файла: {e}")
            database_path = None  # Сброс пути для повторного ввода


def filter_by_status_interactive(transactions: List[Dict]) -> List[Dict]:
    """Интерактивная фильтрация по статусу"""
    while True:
        print("\nВведите статус, по которому необходимо выполнить фильтрацию.")
        print("Доступные для фильтрации статусы: EXECUTED, CANCELED, PENDING")

        status = input().strip().upper()

        if status in ("EXECUTED", "CANCELED", "PENDING"):
            filtered = filter_by_state(transactions, status)
            print(f'Операции отфильтрованы по статусу "{status}"')
            return filtered
        else:
            print(f'Статус операции "{status}" недоступен.')


def sort_by_date_interactive(transactions: List[Dict]) -> List[Dict]:
    """Интерактивная сортировка по дате"""
    print("\nОтсортировать операции по дате? Да/Нет")
    if ask_yes_no():
        print("Отсортировать по возрастанию или по убыванию?")
        ascending = ask_ascending()
        return sort_by_date(transactions, ascending)
    return transactions


def filter_by_currency_interactive(transactions: List[Dict]) -> List[Dict]:
    """Интерактивная фильтрация по рублям"""
    print("\nВыводить только рублёвые транзакции? Да/Нет")
    if ask_yes_no():
        return list(filter_by_currency(transactions, "RUB"))
    return transactions


def search_by_description_interactive(transactions: List[Dict]) -> List[Dict]:
    """Интерактивный поиск по описанию"""
    print("\nОтфильтровать список транзакций по определенному слову в описании? Да/Нет")
    if ask_yes_no():
        phrase = input("Введите слово или фразу для поиска: ").strip()
        return reg_proc_search(transactions, phrase, True, re.IGNORECASE)
    return transactions


def ask_yes_no() -> bool:
    """Вспомогательная функция для вопросов Да/Нет"""
    while True:
        answer = input().strip().lower()
        if answer in ("да", "д", "yes", "y"):
            return True
        elif answer in ("нет", "н", "no", "n"):
            return False
        print('Неверный ввод! Пожалуйста, введите "Да" или "Нет"')


def ask_ascending() -> bool:
    """Вспомогательная функция для выбора порядка сортировки"""
    while True:
        answer = input().strip().lower()
        if "возрастан" in answer:
            return True
        elif "убыван" in answer:
            return False
        print('Неверный ввод! Пожалуйста, введите "по возрастанию" или "по убыванию"')


def print_transactions(transactions: List[Dict]):
    """Вывод результатов пользователю"""
    if not transactions:
        print("\nНе найдено ни одной транзакции, подходящей под ваши условия фильтрации")
        return

    print("\nРаспечатываю итоговый список транзакций...")
    print(f"\nВсего банковских операций в выборке: {len(transactions)}\n")

    for tx in transactions:
        # Форматирование вывода транзакции
        date = tx.get("date", "Дата неизвестна")
        desc = tx.get("description", "Описание отсутствует")

        # Маскировка номеров карт/счетов
        from_acc = mask_account(tx.get("from", ""))
        to_acc = mask_account(tx.get("to", ""))

        # Форматирование суммы
        amount = tx.get("operationAmount", {}).get("amount", "0")
        currency = tx.get("operationAmount", {}).get("currency", {}).get("name", "")

        print(f"{date} {desc}")
        if from_acc:
            print(f"{from_acc} -> {to_acc}")
        else:
            print(f"-> {to_acc}")
        print(f"Сумма: {amount} {currency}\n")


def mask_account(account: str) -> str:
    """Маскировка номера карты/счета"""
    if not account:
        return ""

    if "Счет" in account:
        return f"Счет **{account[-4:]}" if len(account) > 4 else account
    else:
        # Маскировка номера карты (вид: Visa Platinum 1234 56** **** 5678)
        parts = account.split()
        numbers = parts[-1] if parts else ""
        if len(numbers) == 16 and numbers.isdigit():
            masked = f"{numbers[:4]} {numbers[4:6]}** **** {numbers[-4:]}"
            return " ".join(parts[:-1] + [masked])
        return account


# Для тестирования можно использовать mock-объекты
if __name__ == "__main__":
    main()'''